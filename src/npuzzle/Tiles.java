...
public class Tiles implements State {
	...
	// Returns true iff every tile is in the same location
	@Override
	public boolean equals(Object that) {
		if (!(that instanceof Tiles)) 
			return false;					// Tiles can only equal other Tiles
		else {	
	...
	// Returns a hashcode based on the positions of the tiles as its only field
	// Doesn't work with HashTables
	@Override
	public int hashCode() {
		int code = 0;
		for (int i=0; i<width*width; i++) {
			code += (Math.pow(width*width,i))*tiles[i];
		}
		return code;
	}
}
